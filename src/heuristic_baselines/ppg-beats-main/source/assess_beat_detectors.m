...
eval(sprintf('uParams.paths.processing_folder = ''%s%s%s%s%s'';', folder_containing_raw_data_file, filesep, 'ppg-beats-processing-', uParams.dataset(strlength(uParams.dataset)-6:strlength(uParams.dataset)-4), filesep));
...
data = load_data(uParams); % loads data and adjusts length if ecg and ppg are not the same
